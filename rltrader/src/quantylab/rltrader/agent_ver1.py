...
class Agent:
    ...
    def buy_stock(self, price, units):
        """주식 매수 API 호출"""
        try:
            response = requests.post(
                "매수 API 엔드포인트 URL",
                json={"action": "buy", "price": price, "units": units}
            )
            response.raise_for_status()  # HTTP 요청 오류 처리
            data = response.json()
            if data['status'] == 'success':
                return True, data['filled_units']
            return False, 0
        except Exception as e:
            print(f"Buy order failed: {e}")
            return False, 0

    def sell_stock(self, price, units):
        """주식 매도 API 호출"""
        try:
            response = requests.post(
                "매도 API 엔드포인트 URL",
                json={"action": "sell", "price": price, "units": units}
            )
            response.raise_for_status() # HTTP 요청 오류 처리
            data = response.json()
            if data['status'] == 'success':
    ...
    def act(self, action, confidence): 
        ...
        if action == Agent.ACTION_BUY:
            ...
            success, executed_units = self.buy_stock(curr_price, trading_unit)
            if success and executed_units > 0:
                invest_amount = curr_price * (1 + self.TRADING_CHARGE) * executed_units
                self.avg_buy_price = (self.avg_buy_price * self.num_stocks + curr_price * executed_units) / (self.num_stocks + executed_units)
                self.balance -= invest_amount # 보유 현금을 갱신
                self.num_stocks += executed_units # 보유 주식 수를 갱신
                self.num_buy += 1 # 매수 횟수 증가

        elif action == Agent.ACTION_SELL:
            trading_unit = min(self.decide_trading_unit(confidence), self.num_stocks)
            success, executed_units = self.sell_stock(curr_price, trading_unit)
            if success and executed_units > 0:
                invest_amount = curr_price * (1 - (self.TRADING_TAX + self.TRADING_CHARGE)) * executed_units
                self.avg_buy_price = \
                    (self.avg_buy_price * self.num_stocks - curr_price * executed_units) / (self.num_stocks - executed_units) if self.num_stocks > executed_units else 0
                self.num_stocks -= executed_units # 보유 주식 수를 갱신
                self.balance += invest_amount # 보유 현금을 갱신
