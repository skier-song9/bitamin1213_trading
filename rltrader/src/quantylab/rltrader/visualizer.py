...
class Visualizer:
    ...
    def prepare(self, chart_data, title):
        ...
        with lock:
            ...
            # ⚠️ open, high, low, close 순서로된 2차원 배열
            ohlc = np.hstack((
    ...
    def plot(self, epoch_str=None, num_epoches=None, epsilon=None,
            ...
            initial_balance=None, pvs=None, learning_idxes=None):
        '''
        :param epoch_str: Figure 제목으로 표시한 epoch
        :param num_epoches: 총 수행할 epoch 수
        :param epsilon: 탐험률
        :param action_list: agent가 수행할 수 있는 행동 리스트 = action space
        :param actions: agent가 수행한 행동들 리스트 = action history
        :param num_stocks: 주식 보유 수 리스트
        :param outvals_value: 가치 신경망의 출력 리스트
        :param outvals_policy: 정책 신경망의 출력 리스트
        :param exps: 탐험 여부 리스트
        :param initial_balance: 초기 자본금
        :param pvs: 포트폴리오 가치 리스트
        :param learning_idxes: 학습 위치 리스트
        '''
        with lock:
            ...
            for action, color in zip(action_list, self.COLORS):
                for i in self.x[actions == action]:
                    # 배경 색으로 행동 표시 ( 매수 = 빨강, 매도 = 파랑, 관망 = 초록 )
            ...
            '''
            배경색 = 수행한 행동
            노란색 배경 = 탐험(무작위 행동)한 행동
            그 외 = 정책 신경망의 출력값(line plot)에 따라 파란선이 더 위에 있으면 매도행동,
                    빨간선이 더 위에 있으면 매수행동을 수행한 것. 
            '''
            # 탐험을 노란색 배경으로 그리기
            for exp_idx in exps:
