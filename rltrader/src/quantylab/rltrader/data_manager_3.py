...
COLUMNS_TRAINING_DATA_V1 = [
    ...
    #추가지표
    'slowk', 'slowd', 'rsi', 'upper', 'middle', 'lower',
    'macd', 'macd_signal', 'macd_hist', 'ad',
...
def preprocess(data, ver='v1'):
    ...
    #스토케스틱오실레이터 - slowk : 현재의 종가와 일정 기간 동안의 저가와 고가의 범위 비교, slowd : slowk의 이동평균
    stoch = ta.STOCH(data['high'], data['low'], data['close'],\
                 fastk_period = 15, slowk_period = 3, slowk_matype = 0,\
                 slowd_period = 3, slowd_matype = 0)
    stoch[0].fillna(method='bfill', inplace = True)
    stoch[1].fillna(method='bfill', inplace = True)
    data['slowk'] = stoch[0]
    data['slowd'] = stoch[1]
 
    #상대강도지수 - 과열이나 침체 국면 판단
    data['rsi'] = ta.RSI(data['close'], timeperiod = 14)

    #볼린저밴드 - 이동평균선을 중심으로 표준편차 범위 표시
    bbands = ta.BBANDS(data['close'], timeperiod = 20)

    upper = bbands[0].fillna(method = 'bfill')
    middle = bbands[1].fillna(method = 'bfill')
    lower = bbands[2].fillna(method = 'bfill')

    data['upper'] = upper
    data['middle'] = middle
    data['lower'] = lower

    #이동평균수렴확산 - 2개의 장단기 지수이동평균선으로 모멘텀을 추정
    macd = ta.MACD(data['close'], fastperiod = 12, slowperiod = 26, signalperiod = 9)

    macd_0 = macd[0].fillna(method = 'bfill')
    macd_signal = macd[1].fillna(method = 'bfill')
    macd_hist = macd[2].fillna(method = 'bfill')

    data['macd'] = macd_0
    data['macd_signal'] = macd_signal
    data['macd_hist'] = macd_hist

    #누적체적선 - 거래량을 고려하여 가격 움직임을 분석, 거래량의 상대적인 힘을 평가
    data['ad'] = ta.AD(data['high'], data['low'], data['close'], data['volume'])

    #AROON - 가격의 상승 또는 하락 추세의 강도와 지속시간 측정
    aroon = ta.AROON(data['high'], data['low'], timeperiod=14)

    aroon_up = aroon[0].fillna(method = 'bfill')
    aroon_down = aroon[1].fillna(method = 'bfill')

    data['aroon_up'] = aroon_up
    data['aroon_down'] = aroon_down

    #ATR - 가격 움직임의 평균 변동성을 계산하여 트레이딩에 적합한 스톱 로스와 타겟 가격 설정에 사용
    data['atr'] = ta.ATR(data['high'], data['low'], data['close'], timeperiod = 14)

    if ver == 'v0':
        data['stochastic'] = ta.STOCH(data['high'], data['low'], data['close'],\
                                      fastk_period = 15, slowk_period = 3, slowk_matype = 0,\
                                      slowd_period = 3, slowd_matype = 0)
        data['rsi'] = ta.RSI(data['close'], timeperiod = 14)
        data[['upper', 'middle', 'lower']] = ta.BBANDS(data['close'], timeperiod = 20)
        data[['macd', 'macd_signal', 'macd_hist']] = ta.MACD(data['close'],\
                                                             fastperiod = 12,\
                                                             slowperiod = 26,\
                                                             signalperiod = 9)
        data['ad'] = ta.AD(data['hign'], data['low'], data['close'], data['volume'])
        data[['aroon_up', 'aroon_down']] = ta.AROON(data['high'], data['low'],\
                                                    timeperiod=14)
...
def load_data(code, date_from, date_to, ver='v1'):
    ...
    df = pd.read_csv(
        os.path.join(settings.BASE_DIR, 'data', 'v1', f'{code}.csv'),
        thousands=',', converters={'date': lambda x: str(x)})
